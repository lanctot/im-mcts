...
%%% Note: when copying to arXiv paper, do the following: 
  % - add \vspace under the fact subsections in breakthrough section
...
The best playout with this evaluation function is fet$20$ with node priors, which we call the 
alternative baseline, abbreviated bl'.
...
Implicit minimax backups improves performance against a strong benchmark player, even when using a simpler evaluation function. 

We then played the two best players the respective evaluation functions against each other, 
...
%This result could indicate that implicit minimax
%benefits from more defensive and less granular evaluation function in 
%It is possible that implicit minimax backups may benefit more when 
%combined with defensive and less granular evaluation functions in Breakthrough. 
%We try to further explain this finding in the next subsection. \\
...
Lines of Action~\cite{Winands11AB} and multiplayer search~\cite{Nijssen13,Sturtevant08icga}.
