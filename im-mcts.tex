...
To tune parameters in Kalah and Breakthrough, we ran hierarchical elimination tournaments against where each 
head-to-head match consisted of 200 games with seats swapped halfway. 
...
We determined which playout enhancement led to the best baseline player. 
...
\[ \hQ^{CB}(s,a) = (1-\alpha)Q + \alpha v_0(s'),\] 
...
in line 2, with $\alpha = 0.4$ in both cases.
MCTS(bl,im$0.4$) won 67.8\% against MCTS(bl,$\hQ^{CB}$). 
MCTS(bl,im$0.4$) won 65.5\% against MCTS(bl,$\hQ^{PB}$). 
It is possible that a different decay function for the weight placed on $v^{\tau}_s$ could further improve 
the advantage of implicit minimax backups, and we leave this as an interesting topic for future work. 
...
success in Go, and has also been applied with success to path planning problems~\cite{Eyerich10High}.
...
reasons. 
First, in our experience it seems that traps are effectively handled by improving the playout policy. 
Even without early terminations, 
...
than simply detecting and handling traps. 
