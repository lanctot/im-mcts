...
\hline
time $\backslash T$ &   10 &  50 & 100 & 250 & 500 & 1000 & 1500 & 2000 \\
\hline
1s                  &      &     &     &     &     &      &      &      \\
5s                  &      &     &     &     &     &      &      &      \\  
\hline
\end{tabular}
\end{center}
\caption{Win rates (\%) of MCTS(im$0.4$) vs. maximim backpropagation in Breakthrough. \mlnote{Exps still running...}}
...
Results are shown in Figure~\ref{fig:bt-alt-alpha}. In this case, the advantage diminishes and 
the optimal range of $\alpha$ changes to $[0.4,0.55]$. We label the best player in this figure 
using the alternative baseline MCTS(bl', im$0.55$, np). 
...
\begin{figure}[t]
\begin{center}
\includegraphics[scale=0.68]{plots/bt-base-alpha}
\caption{Results of varying $\alpha$ in Breakthrough using the alternative baseline player.
Each point represents 1000 games. \mlnote{Note: This graph is a placeholder for the moment.}} 
