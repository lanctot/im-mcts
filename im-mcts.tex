...
With each new simulation, these estimates improve and help guide future simulations. 
...
that assigns each piece a score of 10 and the further row achieved as 2.5, and the one described by
...
\end{tabular}
\end{center}
\caption{Win rates (\%) of MCTS(im$0.4$) vs. maximim backpropagation in Breakthrough. }
...
We played the two best players against each other, and MCTS(im$0.6$,np) wins with 76.4\%. 
...
For LOA, we have based our implementation on the state-of-the-art techniques described in \cite{Winands10MCTS-LOA}, 
...
and a simple playout that is selects moves non-uniformly at random based on the MIA features, and uses the early cut-off strategy. 
...
categories is added, the advantange diminishes. However, we do notice that $\alpha \in [0.05,0.3]$ seems to not significantly 
decrease the performance. 

The results are summarized in Table~\ref{tbl:loaresults}. From the graph, we reran $\alpha = 0.2$ with progressive bias for 
32000 games giving a statistically significant (95\% confidence) win rate of 50.59\%. We also tried increasing the search time, 
...
implicit minimax backups against an $\alpha \beta$ player based on MIA. When progress bias is disabled, implicit minimax backups
increases the performance by 11 percentage points. There is also notice a small (but statistically insignificant) increase in 
performance when progressive bias is enabled. 
Also, at $\alpha = 0.2$, there is no statistically significant case of implicit minimax backups hurting performance. 
...
basic evaluation functions, in Breakthrough and Kalah. Furthermore, the technique helps 
performance LOA, a larger, more complex domain with sophisticated knowledge and 
